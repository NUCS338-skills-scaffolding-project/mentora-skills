...
def render_progress(observations: list[dict], scope: str) -> dict:
  """Score this student's inference-threats practice for the Progress tab.
  ...
  summed across assignments touched. PROFICIENT_AT = 5.
  ...
  if scope == "course":
    return {
      "status": status,
      "score": score,
      "headline": f"{cumulative} of {PROFICIENT_AT} substantive threats raised",
    }
  else:
    return {
      "status": status,
      "score": score,
      "insight": None,  # caller falls back to _default_insight (skill's row observations)
    }
