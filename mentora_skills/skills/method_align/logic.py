...
def render_progress(observations: list[dict], scope: str) -> dict:
  """Score this student's method-align practice for the Progress tab.
  ...
  summed across assignments touched. PROFICIENT_AT = 8.
  ...
  """
  PROFICIENT_AT = 8  # calibration: review after fall pilot
  ...
  if scope == "course":
    return {
      "status": status,
      "score": score,
      "headline": f"{cumulative} of 8 method concerns logged",
    }
  else:
    return {
      "status": status,
